...
def dft(n):
    ...
    for i in range(n):
        for j in range(n):
            ...
            dft_matrix = np.append(dft_matrix,k1)
    dft_matrix = np.reshape(dft_matrix,(n,n))      
    return dft_matrix   
...
def dct(N):
    ...
    for n in range(N):
        for k in range(N):
            answer = np.cos((np.pi*k*(2*n+1))/(2*N))
            dct_matrix = np.append(dct_matrix, answer)
            
    dct_matrix = np.reshape(dct_matrix,(N,N))   
 
    return dct_matrix
 
    #defining haar matrix
def haar(N):
    final=[]
    if N==2:
...
plt.plot(inputx,final_error, 'g', label='DFT') 
plt.plot(inputx,final_error_dct,'r', label = 'DCT') 
plt.plot(inputx,final_error_haar, 'b', label='HAAR')